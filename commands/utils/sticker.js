...
const buildFFmpegFilters = (effects, isVideo = false) => {
  ...
  if (shape && !['cover', 'contain', 'mirror', 'border', 'frame'].includes(shape)) {
    ...
    switch (shape) {
      ...
      case 'roundrect':
  alphaExpr = `if(between(X,25,${W-25})*between(Y,0,${H}) + between(X,0,${W})*between(Y,25,${H-25}),255, if(lte(hypot(X-25,Y-25),25)+lte(hypot(X-${W-25},Y-25),25)+lte(hypot(X-25,Y-${H-25}),25)+lte(hypot(X-${W-25},Y-${H-25}),25),255,0))`;
  break;
