...
export default {
  command: ['ver', 'read', 'readvo'],
  category: 'tools',
  run: async (client, m, args, usedPrefix, command, text) => {
    const quoted = m.quoted
    if (!quoted) return m.reply( '> 𓈒    ׂ   🐢੭       ᮫      :  Por favor, responde a un mensaje "ViewOnce" para ver su contenido.')
    try {
      await m.react('🕒')
      const content = extractMessageContent(quoted.message || quoted)
      if (!content) return m.reply('> 𓈒    ׂ   ⚠️੭       ᮫      :  No se pudo extraer el contenido.')
      ...
      if (!stream) return m.reply('> 𓈒    ׂ   ⚠️੭       ᮫      :  No se pudo descargar el contenido.')
