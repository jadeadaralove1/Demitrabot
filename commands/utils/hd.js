...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!/^image\/(jpe?g|png|webp)$/i.test(mime)) return m.reply(`> 𓈒    ׂ   🪼੭       ᮫      :  El formato *${mime || 'desconocido'}* no es compatible`)

      const buffer = await q.download?.()
      if (!buffer || !Buffer.isBuffer(buffer) || buffer.length < 10) return m.reply('> 𓈒    ׂ   🦭੭       ᮫      :  No se pudo descargar la imagen')

      const ft = await safeFileType(buffer)
      const inputMime = ft?.mime || mime || 'image/jpeg'
      if (!/^image\/(jpe?g|png|webp)$/i.test(inputMime)) return m.reply(`੭୧   ᮫    ࿀ El formato *${inputMime}* no es compatible`)
