  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (groupAnnouncement === false) {
        return client.reply(m.chat, ` 𝄄ׄㅤ𝅄⚠️⃞፝͜͡⌒𝅄  El grupo ya está abierto.`, m)
      }
      const applyAction = async () => {
        await client.groupSettingUpdate(m.chat, 'not_announcement')
        return client.reply(m.chat, `(＼᳟ㅤ ⃞🔓ㅤ.ᐟ   𝗚𝗋𝗎𝗉𝗈 𝖠𝖻𝗂𝖾𝗋𝗍𝗼. 
✧  ׁ    𓈒    
> 𝗘𝗌𝗍𝖾 𝗀𝗋𝗎𝗉𝗈 𝖿𝗎𝖾 𝖺𝖻𝗂𝖾𝗋𝗍𝗈, 𝖫𝗈𝗌 𝗂𝗇𝗍𝖾𝗀𝗋𝖺𝗇𝗍𝖾𝗌 𝗒𝖺 𝗉𝗎𝖾𝖽𝖾𝗇 𝗁𝖺𝖻𝗹𝗮𝗿.`, m)
      }
      if (timeout > 0) {
        await client.reply(m.chat, `𝄄ׄㅤ𝅄⚠️⃞፝͜͡⌒𝅄  El grupo se abrirá en ${clockString(timeout)}.`, m)
        setTimeout(async () => {
          try {
