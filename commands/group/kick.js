export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    if (!m.mentionedJid[0] && !m.quoted) {
      return m.reply('𝄄ׄㅤ𝅄⚠️⃞፝͜͡⌒𝅄  Etiqueta o responde al *mensaje* de la *persona* que quieres eliminar')
    }
    ...
    if (user === client.decodeJid(client.user.id)) {
      return m.reply('𝄄ׄㅤ𝅄⚠️⃞፝͜͡⌒𝅄  No puedo eliminar al *bot* del grupo')
    }
    if (user === ownerGroup) {
      return m.reply('𝄄ׄㅤ𝅄🪷⃞፝͜͡⌒𝅄 No puedo eliminar al *propietario* del grupo')
    }
    if (user === ownerBot) {
      return m.reply('𝄄ׄㅤ𝅄💮⃞፝͜͡⌒𝅄 No puedo eliminar al *propietario* del bot')
    }
    try {
